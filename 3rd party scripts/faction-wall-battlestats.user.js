...
let loadStatsBacklog = [];
function loadStats(id, node) {
	if (!apiKey) return;

	if (loadStatsLock) {
		if (id) loadStatsBacklog.push([id, node]);
		return;
	}
	...
	if (!id) {
		if (loadStatsBacklog.length == 0) {
			loadStatsLock = false;
			return;
		}
		else {
			let backlog = loadStatsBacklog.shift();
			id = backlog[0];
			node = backlog[1];
		}
	}

	function loadStatsDone(t = 0){
		setTimeout(() => {
			loadStatsLock = false;
			loadStats();
		}, t);
	...
	GM_xmlhttpRequest({
		method: "GET",
		url: `https://www.tornstats.com/api/v1/${apiKey}/spy/${id}`,
		onload: (r) => {
			if (r.status == 429) {
				let headers = r.responseHeaders.split("\r\n").map((h) => h.split(":"));
				let retry = 60;
				headers.forEach((h) => {
					if (h[0].toLowerCase() != "retry-after") return;
					retry = ~~h[1];
				}); 
				loadStatsDone(retry*1000);
				return;
			}
			...
			let j = JSONparse(r.responseText);
			if (!j || !j.spy || !j.spy.status) return;

			bsCache[id] = j.spy;
			bsCache[id].timestamp = new Date().getTime();
			localStorage["finally.torn.bs"] = JSON.stringify(bsCache);

			updateStats(id, node);
		},
		onabort: () => loadStatsDone(),
		onerror: () => loadStatsDone(),
		ontimeout: () => loadStatsDone()
	});
}
...
function showStats(node) {
	...
	if (!bsCache[id] || bsCache[id].timestamp <= (new Date()-(24*60*60*1000))) {
		loadStats(id, bsNode);
	}
...
function showStatsAll(node) {
	...
	node.querySelectorAll(".your:not(.row-animation-new), .enemy:not(.row-animation-new)").forEach((e) => showStats(e));
}
...
function watchWall(observeNode) {
	...
	if (!titleNode.querySelector(".bs")) {
		...
		let active = sortIcon.className.indexOf("activeIcon") !== -1;
