// ==UserScript==
// @name        Carbon: Modified Faction Wall Battle Stats script (originally by finally)
// @description Show tornstats spies on faction wall page (fixed illegal use of websockets by Heasley)
...
function updateStats(id, node, parentNode) {
	...
	<div class="finally-bs-stat">
		...
		<b>TOT</b> <span class="finally-bs-stat">${stats[0]}</span><br/>
	</div>`;
...
function showStats(node) {
    ...
    bsNode.addEventListener("click", async () => {
        if (isMobile) return;
        await navigator.clipboard.writeText(
        `https://www.torn.com/loader.php?sid=attack&user2ID=${id}
        TOT: ${bsCache[id].total.toLocaleString()}
        MAX PARTICIPANTS: ${calculateParticipantsRequired(bsCache[id].total)}`);
        //    alert(`Copied attack URL to clipboard! ID ${id}`);
        //   window.open(`loader.php?sid=attack&user2ID=${id}`, "_newtab");
    });
    bsNode.addEventListener("dblclick", () => {
        navigator.clipboard.writeText(`https://www.torn.com/loader.php?sid=attack&user2ID=${id}`);
        // alert(`Copied attack URL to clipboard!`);

        //window.open(`loader.php?sid=attack&user2ID=${id}`, "_newtab");
    });
}

function calculateParticipantsRequired(total) {

    let x = 0;
    let effective = total;
    let threshold = 2500000000;

    while (effective > threshold) {
...
unsafeWindow.WebSocket = function(...args) {
    ...
    socket.addEventListener("message", (event) => {
        if (document.hasFocus() && !document.hidden) { //checks for tab active / visible / focused
